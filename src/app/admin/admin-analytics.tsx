...
function computeKitchenMetrics(orders: any[], kitchenIds: string[]) {
  ...
  mine.forEach(o => {
    const id = o.user?._id || o.user?.name;
  ...
  return {
    ...
    acceptanceRate: mine.length ? `${Math.round((delivered.length / mine.length) * 100)}%` : '0%',
    rating: '4.6',
  };
}
...
export function OverviewTab({
  ...
  const ownerKitchenIds = React.useMemo(() => {
    ...
    return kitchens
      .filter((k: any) => (k.owner?._id || k.owner) === user?._id || k.owner === user?._id)
      .map((k: any) => k._id);
  ...
  return (
    <div className={ERP.page}>
      ...
      <AdminPageHeader
        ...
        subtitle={isKitchenOwner ? 'Kitchen performance — scoped to your operations' : 'Platform-wide reporting and operational metrics'}
        actions={
          <>
            <button type="button" className={`${ERP.btn} ${ERP.btnSecondary}`} title="Saved presets"><Bookmark className="w-3.5 h-3.5" /> Presets</button>
            <button type="button" className={`${ERP.btn} ${ERP.btnSecondary}`} title="Schedule reports"><Clock className="w-3.5 h-3.5" /> Schedule</button>
            <button
              type="button"
              onClick={() => exportToCsv(
      ...
      <div className={`${ERP.panel} p-3 space-y-3`}>
        ...
        {liveUsers > 0 && (
          <p className="text-[10px] font-semibold text-slate-500 flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-green-600" /> {liveUsers} active sessions · real-time
          </p>
        )}
      </div>

      <AdminKpiCarousel items={isAdmin ? adminKpis : ownerKpis} />
      <div className="hidden md:block">
        <AdminKpiGrid items={isAdmin ? adminKpis : ownerKpis} className="!grid-cols-2 md:!grid-cols-4 lg:!grid-cols-6" />
      </div>
      ...
      <AdminPanel
        title="Report table"
        subtitle="Excel-style export · sort · search"
        actions={
          <select value={reportType} onChange={e => setReportType(e.target.value)} className={`${ERP.select} w-auto text-xs`}>
            <option value="orders">Orders report</option>
            {isAdmin && <option value="kitchens">Kitchen ranking</option>}
      ...
      >
        <AdminDataTable
          columns={reportColumns[reportType] || reportColumns.orders}
          rows={tableRows}
          emptyMessage="No data for selected filters"
          exportFilename={`report-${reportType}.csv`}
          exportHeaders={(reportColumns[reportType] || reportColumns.orders).map(c => c.header)}
          exportRows={tableRows.map((r: any) =>
            (reportColumns[reportType] || reportColumns.orders).map(c => {
              const v = c.render ? undefined : (r as Record<string, unknown>)[c.key];
      ...
      {isAdmin && stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          {[
            ...
            { label: 'Menu items', value: stats.menuItems, Icon: Package },
            { label: 'Subscriptions', value: stats.subscriptions, Icon: BarChart2 },
            { label: 'Kitchens', value: kitchens.length, Icon: Store },
          ].map(s => (
            <div key={s.label} className={`${ERP.kpi} flex items-center gap-2`}>
              <s.Icon className="w-4 h-4 text-slate-400 shrink-0" />
              <div><p className={ERP.kpiLabel}>{s.label}</p><p className={ERP.kpiValue}>{s.value}</p></div>
            </div>
          ))}
        </div>
      )}
